(function() {
    ...
    async function fetchDialogExtensions(target, payload) {
        const host = window.parent && window.parent !== window ? window.parent : window;
        if (!host.JSOSPluginsHost || typeof host.JSOSPluginsHost.getExtensions !== 'function') {
            return [];
        }

        try {
            const extensions = await host.JSOSPluginsHost.getExtensions(target, payload || {});
    ...
    async function executeDialogExtension(extension, context) {
        const host = window.parent && window.parent !== window ? window.parent : window;
        if (!host.JSOSPluginsHost || typeof host.JSOSPluginsHost.executeExtension !== 'function') {
            throw new Error('Plugin host is not available');
        }

        await host.JSOSPluginsHost.executeExtension(extension, context || {});
    }
        ...
        async function showFileDialog(options) {
            ...
            return new Promise(function(resolve) {
                async function render() {
                    ...
                    pluginActions.className = 'jsos-dialog-actions';
                    pluginActions.style.padding = '0';
                    pluginActions.style.borderTop = 'none';
                    pluginActions.style.background = 'transparent';
                    body.appendChild(pluginActions);
                    ...
                    try {
                        ...
                        const extensions = await fetchDialogExtensions('dialogs.file', {
                            mode,
                            path: currentPath,
                            selectedPath
                        });
                        ...
                        extensions.forEach(function(extension) {
                            ...
                            pluginButton.className = 'jsos-dialog-button';
                            pluginButton.textContent = extension.label || extension.id || 'Plugin Action';
                            pluginButton.addEventListener('click', async function() {
                                try {
                                    await executeDialogExtension(extension, {
                                        mode,
                                        path: currentPath,
                                        selectedPath
                                    });
                            ...
                            pluginActions.appendChild(pluginButton);
                        });
